...
# March. 10, 2025
# This script performs Linear Mixed Model for Edges in the Right Hemisphere
...
# modify line below for different hemisphere
   file_pattern<- paste0(subject_id,"_jackknife_sigConn_aparc35_node2vec/",subject_id,"_jackknife_sigConn_aparc35_RH_analysis_results.xlsx")
...
combined_data_RH$SSAGA_Educ.2<- as.character(combined_data_RH$SSAGA_Educ.2)
head(combined_data_RH)

# merge Source and Target columns into one
new_data_RH <- combined_data_RH %>%
      mutate(Edges = paste(Source, Target, sep = "-"))
      

head(new_data_RH)
Weight<- new_data_RH$Weight
write.csv(new_data_RH, file="combined_RH_Edges_2.csv")
...
print(results_RH_edges)
results_RH_edges<- results_RH_edges[, !sapply(results_RH_edges, is.list)]
write.csv(results_RH_edges, file="RH_LModel_Edges_31025.csv")

# Visualization with error bars and p-values
plot_RH_Edges_data <- new_data_RH %>%
...
ggplot(new_data_RH, aes(x = Edges, y = Weight, fill = Gender)) +
  ...
  labs(
    title = "Gender Differences for Brain Connections in the Right Hemisphere",
    x = "Brain Connections in the Right Hemisphere",
    y = "Mean ± SE"
  ) +
  theme_minimal() +
  theme(
  ...
  geom_text(data = results_RH_edges, aes(x = Edges, y = max(new_data_RH$Weight) + 0.2,
                                label = ifelse(PValue < 0.05, "*", "")),
              inherit.aes = FALSE)
             #inherit.aes = FALSE, size = 5, color = "black")

ggsave(file = "RH_Edges.jpg", dpi = 300)


# Filter
sig_Edges<- results_RH_edges %>% filter(PValue<0.05) %>% pull (Edges)

#Subset
edges_filtered<- new_data_RH %>% filter(Edges %in% sig_Edges)

# stats for filtered dat
filter_RH_Edges <- edges_filtered %>%
  group_by(Edges, Gender) %>%
  summarise(
    Mean = mean(Weight, na.rm = TRUE),
    SE = sd(Weight, na.rm = TRUE) / sqrt(n()),
...
ggplot(edges_filtered, aes(x = Edges, y = Weight, fill = Gender)) +
  geom_violin(alpha=0.6, trim = FALSE, position = position_dodge(0.8)) +
  geom_boxplot(width=0.2, position=position_dodge(0.8), outlier.shape=NA, alpha=0.8)+
  #geom_bar(stat = "identity", position = position_dodge(0.8), width = 0.7) +
  #geom_errorbar(data=plot_RH_Edges_data, aes(x= Edges,ymin = Mean - SE, ymax = Mean + SE, color=Gender, group=Gender),
  ...
  scale_color_manual(values=c("Male" = "blue", "Female" = "red")) +
  labs(
    title = "Gender Differences in the Right Hemisphere",
    x = "Brain Connections in the Right Hemisphere",
...
ggsave(file = "sig_RH_Edges.jpg", dpi = 300)
